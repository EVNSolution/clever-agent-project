...
REQUIRED_FIELDS = ("repo", "issue_number", "work_type", "title")
WORK_TYPE_LABELS = {
    "신규 개발": "신규",
    "신규": "신규",
    "수정": "수정",
...
def shorten_work_type_label(work_type: str) -> str:
    normalized = " ".join(work_type.split())
    return WORK_TYPE_LABELS.get(normalized, normalized)


def normalize_title_body(title: str) -> str:
    normalized = " ".join(title.split()).rstrip(".!? ")
...
class IssueDraft:
    ...
    def formatted_title(self) -> str:
        work_type = shorten_work_type_label(self.work_type)
        title = truncate_title_body(normalize_title_body(self.title))
        return f"{work_type}/{title}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Title format is '<short_work_type>/<short_title>'."
        )
    )
