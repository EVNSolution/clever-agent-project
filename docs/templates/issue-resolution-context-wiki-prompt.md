...
`dev` 또는 `main` PR이 있으면 PR 검토 에이전트 작업은 wiki/service context 업데이트로 마친다.
...
- `dev` 또는 `main` PR 검토 에이전트가 wiki/service context 업데이트로 작업을 마치기 전
...
해결 완료로 표시하기 전, 아래 기준으로 `clever-context-monorepo`의 context 문서 반영 필요 여부를 확인하고, 연결된 `dev` 또는 `main` PR의 검토 에이전트 작업을 wiki/service context 업데이트로 마쳐줘.
...
- 반영 결과를 PR 검토 완료 결과의 `wiki/service context update result`, `service doc update`, `wiki update`, `clever-context-monorepo update`에 남긴다.
- 반영이 불필요하면 왜 불필요한지 PR 검토 완료 결과와 이슈 종료 코멘트에 남길 문장으로 정리
...
- `dev` 또는 `main` PR이 있으면 context/wiki 판단은 PR 검토 에이전트의 종료 작업으로 처리한다.
- 이슈 종료 코멘트에는 PR 검토 완료 결과와 context 문서 반영 여부 또는 불필요 사유를 남긴다.
